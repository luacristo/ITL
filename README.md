...
- Все файлы проекта должны быть организованы в одной директории:
  ...
  - `api.py` - API, Swagger UI
- код должен соответствовать PEP
- каждая функция или метод должны содержать docstring, typehints
- используйте миграции для обеспечения безопасности структуры бд.

### Базово о Миграциях...
**Миграция** — это процесс изменения структуры базы данных (например, добавление новых столбцов, таблиц) без потери данных. Это важно, чтобы удобно и безопасно обновлять базу данных, особенно когда приложение развивается и меняются модели.

_Как это работает:_
- Вы изменяете модели (например, добавляете новый столбец).
- Создаёте миграцию с помощью команды.
